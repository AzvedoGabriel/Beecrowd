...
public class NotasMoedasVersaoDois {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < notas.length; i++) {
            System.out.println(qtdNotas[i] + " nota(s) de R$ " + NumberFormat.getCurrencyInstance(Locale.US).format(notas[i]).replace("$",""));
        }
        System.out.println("MOEDAS:");
        for (int i = 0; i < moedas.length; i++) {
            System.out.println(qtdMoedas[i] + " moeda(s) de R$ " + NumberFormat.getCurrencyInstance(Locale.US).format(moedas[i]).replace("$",""));
        }
